...
resource "null_resource" "rest_proxy" {
  ...
  provisioner "file" {
    content = "${element(data.template_file.rest_props.*.rendered,count.index)}"
    destination = "/tmp/rest-proxy.properties"
  }

  provisioner "file" {
    source = "configs/rest-proxy.service"
    destination = "/tmp/rest-proxy.service"
  }

  provisioner "remote-exec" {
    inline = [
      "sudo mv /tmp/rest-proxy.properties /kafka/etc/",
      "sudo mv /tmp/rest-proxy.service /etc/systemd/system/"
    ]
  }

  provisioner "remote-exec" {
    inline = [
      "sudo systemctl enable rest-proxy",
      "sudo systemctl restart rest-proxy"
    ]
  }
}
